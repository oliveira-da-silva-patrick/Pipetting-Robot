...
class UI:
    ...
    def __init__(self):
        ...
        # minimum dimension, vertical margin, left margin, right margin, image size
        self.layout = [min(self.canvas.winfo_width(), self.canvas.winfo_height()),
                       ...
                       -1, ("Engel", 18, "bold")]
        self.layout[4] = int (0.3 * self.layout[0])
        ...
        # keeps the window alive and running. it is necessary
        self.window.mainloop()

    def setup_application_window(self):
        """configures the application window and its canvas"""
        # application uses 100% of screenwidth
        width = int (1 * self.window.winfo_screenwidth())
        # application uses 94% of screenheight. using more causes problems on raspbian
        height = int (0.94 * self.window.winfo_screenheight())
        # the application is opened on the top left corner of the window
        # geometry = width x height + startX + startY
        geometry = f"{width}x{height}+{0}+{0}"
        self.window.geometry(newGeometry=geometry)
    ...
    def create_art_area(self):
        ...
        start_y = self.canvas.winfo_height() - self.layout[1] - side_length
    ...
    def load_logo(self):
        ...
        # place the image
        pos_y = self.layout[1]
        pos_x = self.canvas.winfo_width() - self.layout[3] - self.layout[4]
    ...
    def mouse_drag_event(self, event):
        ...
        event_widgets = self.canvas.find_overlapping(event.x, event.y, event.x, event.y)
    ...
    def __create_info_window(self):
        ...
        self.widgets[2] = tk.Label(master=self.widgets[1], text=info)
        self.widgets[2].pack()
        # create a button
